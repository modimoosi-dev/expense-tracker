        ...
        <div class="w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            <form id="login-form" class="px-6 pb-6 pt-4 space-y-4" onsubmit="handleLogin(event)">
                ...
                <button type="button" onclick="handleGoogleLogin()"
                   class="w-full flex items-center justify-center gap-3 py-3.5 bg-white border border-gray-300 rounded-xl shadow-sm active:scale-95 transition-all font-medium text-gray-700">
                ...
                async function handleGoogleLogin() {
                    const url = '{{ route('auth.google') }}?redirect_to=app';
                    if (window.Capacitor) {
                        const { Browser } = await import('@capacitor/browser');
                        await Browser.open({ url, presentationStyle: 'popover', toolbarColor: '#4f46e5' });
                    } else {
